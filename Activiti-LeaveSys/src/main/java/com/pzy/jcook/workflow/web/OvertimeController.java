...
public class OvertimeController extends AbstractBaseCURDController<Overtime,Long> {
	...
	public String create(Model model) {
		...
		return "workflow/overtime/create";
	}
	@RequestMapping(value = "create", method = RequestMethod.POST)
	public String create(Overtime ship,Model model) {
		
		User user = (User) SecurityUtils.getSubject().getSession().getAttribute("currentUser");
		ship.setCreater(user);
		ship.setCreateDate(new Date());
		overtimeService.save(ship);
		/** 发起流程 */
		Map<String, Object> activtiMap = new HashMap<String, Object>();
		activtiMap.put("title", ship.getCreater().getChinesename()+"于"+DateFormatUtils.format(new Date(), "yyyy-MM-dd")+"发起的加班单");
		...
		ProcessInstance processInstance = processEngine.getRuntimeService().startProcessInstanceByKey("overtime",
	...
	public ModelAndView task(@PathVariable String taskid, @PathVariable String prcessInstanceid) {
		...
		ModelAndView mav = new ModelAndView("workflow/overtime/" + taskkey);
		...
		mav.addObject("bean", overtimeService.find(Long.valueOf(businessKey)));
	...
	public ModelAndView taskhistory(@PathVariable String prcessInstanceid) {
		...
		ModelAndView mav = new ModelAndView("workflow/overtime/history");
		mav.addObject("taskhistory", workFlowService.findHistoryTask(prcessInstanceid));
		mav.addObject("bean", overtimeService.find(Long.valueOf(bussnessid)));
	...
	public String doapprove(@PathVariable String taskid, @PathVariable String prcessInstanceid, Boolean pass,
			String approvals, Overtime ship,
			RedirectAttributes redirectAttributes) {
		ProcessInstance processInstance = processEngine.getRuntimeService().createProcessInstanceQuery()
				.processInstanceId(prcessInstanceid).singleResult();
		Task task = processEngine.getTaskService().createTaskQuery().taskId(taskid).singleResult();
		Map<String, Object> activtiMap = new HashMap<String, Object>();
		ship = this.overtimeService.find(ship.getId());
		...
		if ("usertask2".equals(task.getTaskDefinitionKey())) {
			...
			this.overtimeService.save(ship);
		...
		return "redirect:/workflow/overtime/taskhistory/" + prcessInstanceid;
	}
	...
	public Response listall(Integer start, Integer length, Long deptment,String username,Date datefrom,Date dateend) {
		...
		int pageSize = length;
		Page<Overtime> m = this.overtimeService.findAll(pageNumber, pageSize, deptment,username,datefrom,dateend);
		return new DataTableResponse<Overtime>( m.getContent(),(int) m.getTotalElements() );
	}
	
	
	@Override
	protected String getBasePath() {
		return "workflow/overtime";
	}

	@ModelAttribute
	public Overtime preget(@RequestParam(required=false) Long id,@RequestParam(required=false) String role) {
		Overtime user = new Overtime();
		if (id!=null){
